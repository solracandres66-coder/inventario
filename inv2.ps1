...
# ====================== DATOS PARA GOOGLE SHEETS (Mejorado) ======================
$data = @{
    computerID         = $computer
    equipoAsociadoID   = ""
    tipoEquipo         = "Desktop"
...
try {
    ...
    $totalRAM = [math]::Round(($ramInfo | Measure-Object Capacity -Sum).Sum / 1GB, 0)
    $data.ram = "$totalRAM GB"
    $data.ramSlots = $ramInfo.Count
} catch {}
...
try {
    ...
    $data.gpu = ($gpus | ForEach-Object { $_.Name }) -join " + "
} catch {}
...
try {
    ...
    $data.monitor = $mon -join " | "
} catch {}

# Licencias Windows y Office
try {
    $winLic = Get-CimInstance SoftwareLicensingProduct -ErrorAction SilentlyContinue | 
...
try {
    $officeLic = Get-CimInstance SoftwareLicensingProduct -ErrorAction SilentlyContinue | 
                 Where-Object { $_.Name -like "*Office*" -and $_.PartialProductKey } | Select-Object -First 1
    if ($officeLic) { $data.licenciaOffice = "$($officeLic.Name) - $($officeLic.LicenseStatus)" }
} catch {}
...
try {
    ...
    Write-Host "⚠️ Error al enviar a Google Sheets" -ForegroundColor Yellow
}
